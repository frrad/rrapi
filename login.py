...
def login(email, password):
    ...
    )  # establish the first connection and get the required cookies and values from the dom and headers

    response = send_login_request(
        soup, email, password, cookie
    )  # send a post request with all the information
...
def establish_first_connection():
    ...
    royalroad_session_id = (
        "372508318544cb51a44a490a91a69ff4"  # actually get this value somewhere
    )
    ...
    cookie = (
        ...
        + "."
        + timestamp
        + "; visited=1; RoyalRoad.SessionId="
        + royalroad_session_id
        + "; "
        + set_cookies[-4].strip().split()[-1]
    )
...
def send_login_request(soup, email, password, cookie):
    ...
    url = "https://www.royalroad.com/account/login"
    rememberme = "false"
    requesttoken = soup.find("input", attrs={"name": "__RequestVerificationToken"}).get(
        "value"
    )
...
def get_logged_in_users_id(login_details):
    ...
    soup = BeautifulSoup(html, "lxml")
    user_id = (
        soup.find("ul", attrs={"class": "dropdown-menu dropdown-menu-default"})
        .find("a")
        .get("href")
        .split("/")[-1]
        .strip()
...
def do_secure_post(
    login_object, token_url, post_url, post_data
):  # only deletes at the moment, need to figure out a good way to reuse code for sending messages and other post request, maybe send post data to this, it grabs security token, and combines?
...
def extract_message_content(soup):
    try:
        ...
        title = soup.find(
            "h3", attrs={"class": "margin-top-0 margin-bottom-10"}
        ).text.strip()
        ...
        print(
            "That message has been deleted, it does not exist, or you do not have permission to view it."
        )
...
def extract_notification_content(soup, notifications):
    ...
    for notification_listing in notification_listings:
        notification_id = (
            notification_listing.find("span", attrs={"class": "dismiss-notification"})
        ...
        content = notification_listing.find(
            "span", attrs={"class": "col-xs-8 col-sm-9"}
        ).text.strip()
        ...
        notifications.append(
            [notification_id, notif_type, url, image_url, content, time]
        )
    return notifications
...
def mark_notifications_as_read(
    login_object,
):  # removes the alert that there is new notifications (that's it)
    if login_object == None:
...
def delete_notification(login_object, notification_id):
    if login_object == None:
        ...
        url = "https://www.royalroad.com/notifications/dismiss?id=" + str(
            notification_id
        )  # 1021667"
        soup = request_secure_page(url, login_object)
...
def rate_fiction(login_object, fiction_id, rating):  # doesn't work
    if login_object == None:
        ...
        post_data = {"id": fiction_id, "rating": rating}
        title = do_secure_post(login_object, token_url, post_url, post_data)
        print(title)
        status = True
        return status
